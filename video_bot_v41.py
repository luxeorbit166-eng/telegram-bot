...
def run_ffmpeg(cmd, timeout=400):
    try:
...
def process_enhance(inp):
    """
    Premium Enhance Ultra — 5-stage cinematic processing:
    1. Advanced denoise (hqdn3d + nlmeans lite)
    2. Smart upscale (if < 1080p)
    3. Cinematic color grading (contrast + sat + gamma + curves)
    4. Dual-stage sharpening (unsharp + detail enhance)
    5. Vignette + film grain
    Audio: EBU R128 loudness normalization + dynamic range
    """
    out = tmp_path(".mp4")
    ...
    vf = (
        f"hqdn3d=3.5:3.5:7:7,"           # Stage 1: Denoise
        f"{scale_f}"                       # Stage 2: Upscale
        f"eq=contrast=1.10:brightness=0.02:saturation=1.30:gamma=0.95,"  # Stage 3: Color
        f"unsharp=5:5:1.5:5:5:0.6,"      # Stage 4a: Sharpen
        f"unsharp=3:3:0.8:3:3:0.3,"      # Stage 4b: Detail
        f"vignette=PI/4.5"               # Stage 5: Vignette
    )

    if has_audio(inp):
        af = "loudnorm=I=-16:TP=-1.5:LRA=11,acompressor=threshold=0.089:ratio=9:attack=200:release=1000"
        cmd = ["ffmpeg","-y","-i",inp,
               "-vf", vf, "-af", af,
...
def main_menu():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📥  ডাউনলোড", callback_data="dl_help")],
        [InlineKeyboardButton("━━━━━━━━━━━━━━━━━━━━━━", callback_data="noop")],
        [InlineKeyboardButton("✨  Enhance ULTRA", callback_data="enhance_help"),
         InlineKeyboardButton("📺  Platform", callback_data="platform_menu")],
        [InlineKeyboardButton("━━━━━━━━━━━━━━━━━━━━━━", callback_data="noop")],
        [InlineKeyboardButton("🎯  WinGo ULTRA Prediction", callback_data="wingo_menu")],
        [InlineKeyboardButton("━━━━━━━━━━━━━━━━━━━━━━", callback_data="noop")],
    ])
...
async def handle_cb(u, c):
    ...
    if data == "wingo_predict":
        ...
        # Confidence bar
        bar_len = 10
        filled  = round(pred["conf"]/100*bar_len)
        ...
        txt = (
            ...
            f"🎨 *Color Prediction:*\n"
            f"  {te} *{pred['tc'].upper()}* সবচেয়ে likely\n"
            f"  🔴`{pred['pR']}%` 🟢`{pred['pG']}%` 🟣`{pred['pV']}%`\n\n"
            f"🎲 *Number Prediction:*\n"
            f"  ★ TOP: `{pred['numTop']}`\n"
            f"  Backup: `{pred['numAlt'][0]}` `{pred['numAlt'][1]}` "
            f"`{pred['numAlt'][2]}` `{pred['numAlt'][3]}`\n\n"
            f"🧠 *Layer Votes:*\n"
            f"  BIG: `{pred['big_layers']}` | SMALL: `{pred['small_layers']}` / `{pred['total_layers']}`\n\n"
            f"📅 Period: `{pred['period'][-6:] if pred['period'] else '—'}`\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"⚠️ _Random game — এটি pattern analysis মাত্র।_"
        )
    ...
    if data == "wingo_auto_on":
        ...
        await safe_edit(q.message,
            ...
            "প্রতি নতুন period এ prediction আসবে।\n"
            "বন্ধ করতে Auto OFF চাপুন।",
            parse_mode="Markdown",reply_markup=wingo_menu()); return

    if data == "wingo_auto_off":
        _wingo_subs.discard(uid)
        await safe_edit(q.message,
            "⏹ *Auto Prediction বন্ধ।*",
            parse_mode="Markdown",reply_markup=wingo_menu()); return
