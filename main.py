...
def process_watermark(inp: str) -> tuple:
    ...
    3. Facebook viral color grade (warm + contrast + sharpen)
    4. Enhance (denoise + brightness)
    5. Copyright bypass (pitch shift + tiny speed + hue)
    6. Text watermark — নিচে ডানে "Bangla brand music"
    """
    out = tmp_path(".mp4")

    # ── Video filter chain ──
    # Step 1-2: mirror + 4:5
    # Step 3: Facebook viral — warm tone, contrast boost
    # Step 4: enhance — denoise + sharpen
    vf = (
        "hflip,"
        ...
        # text watermark — নিচে ডানে, সাদা + হালকা shadow
    ...
    if has_audio(inp):
        af = (
            ...
            f"atempo={speed},"
            f"equalizer=f=8000:width_type=o:width=1.5:g=1.2,"
        ...
        # audio filter fail হলে simple retry
        if not ok:
            cmd = [FFMPEG,"-y","-i",inp,
                   "-vf", vf,
                   "-c:v","libx264","-preset","fast","-crf","18",
    ...
    # সব fail হলে শুধু mirror+4:5 দিয়ে basic retry
    if not ok:
        vf_basic = (
            "hflip,"
            "scale=1080:1350:force_original_aspect_ratio=decrease,"
