...
BOT_TOKEN    = os.getenv("BOT_TOKEN") or "8713772697:AAHX3M6SVB7Av7ZMXTw6u6QMJ_Rx60_0AR0"
...
async def _fetch_1min() -> list:
    ...
    try:
        url = f"https://draw.ar-lottery01.com/WinGo/WinGo_1M/GetHistoryIssuePage.json?ts={int(asyncio.get_event_loop().time()*1000)}"
        r = req.urlopen(url, timeout=8)
        data = json.loads(r.read())
        raw = data.get("data", {}).get("list", [])
        return [{"number": int(x["number"]) % 10,
                 "size": _classify(int(x["number"]) % 10),
                 "dots": _dots(int(x["number"]) % 10),
                 "period": x["issueNumber"]} for x in raw]
    except Exception as e:
        logger.warning("1min fetch error: %s", e)
        return []

def _ultimate_predict(history: list) -> dict:
    """
    ULTIMATE UNIVERSAL PREDICTION LOGIC v3.0
    HTML widget এর JavaScript UltimatePredictor এর exact Python port
    25+ Patterns | Adaptive Weighting | Multi-layer Analysis
    """
    if len(history) < 3:
    ...
    dots    = [get_dots(n) for n in numbers]
    ...
    breaker = "SMALL" if sizes[0] == "BIG" else "BIG"

    # ── 2. Stats ──────────────────────────────────────────
    big_c  = sizes[:20].count("BIG")
    sml_c  = sizes[:20].count("SMALL")
    total  = big_c + sml_c or 1
    imb    = abs(big_c - sml_c)
    dominant = "BIG" if big_c > sml_c else "SMALL"

    # ── 3. Pattern detection (same weights as JS) ─────────
    ...
    W = {
        ...
        "clusterBreak": 77, "zigzagPattern": 69, "boundaryHit": 73,
        "statisticalImbalance": 40, "dotTransition": 59,
        "numberSumPattern": 61, "oscillationDetection": 66,
    }

    L2, L3, L4, L5 = sizes[:2], sizes[:3], sizes[:4], sizes[:5]
    nums8 = numbers[:8]
    dots6 = dots[:6]
    ...
    # Pattern 1: Double Small → Big
    if L2 == ["SMALL","SMALL"]:             add("BIG",   W["doubleSmall"],       "Double Small")
    # Pattern 2: Triple Big → Small
    if L3 == ["BIG","BIG","BIG"]:           add("SMALL", W["tripleBig"],         "Triple Big")
    # Pattern 3: Quadruple streak
    if L4.count("BIG")   == 4:              add("SMALL", W["quadrupleStreak"],   "Quadruple BIG")
    if L4.count("SMALL") == 4:              add("BIG",   W["quadrupleStreak"],   "Quadruple SMALL")
    # Pattern 4: Quintuple streak
    if L5.count("BIG")   == 5:              add("SMALL", W["quintupleStreak"],   "Quintuple BIG")
    if L5.count("SMALL") == 5:              add("BIG",   W["quintupleStreak"],   "Quintuple SMALL")
    # Pattern 5: Alternation
    if L4 == ["BIG","SMALL","BIG","SMALL"]: add("BIG",   W["perfectAlternation"],"Perfect Alternation")
    if L4 == ["SMALL","BIG","SMALL","BIG"]: add("SMALL", W["reverseAlternation"],"Reverse Alternation")
    # Pattern 6-7: Zero / Nine boundary
    if nums8[0] == 0:
        add("BIG",   W["zeroDetection"], "Zero Detected")
        add("BIG",   W["boundaryHit"],   "Boundary Hit 0")
    if nums8[0] == 9:
        add("SMALL", W["nineDetection"], "Nine Detected")
        add("SMALL", W["boundaryHit"],   "Boundary Hit 9")
    # Pattern 8: Number cluster break
    from collections import Counter as _C
    nc = _C(nums8[:5])
    for num, cnt in nc.items():
        if cnt >= 3:
            opp = "SMALL" if num >= 5 else "BIG"
            add(opp, W["clusterBreak"], f"Cluster Break {num}")
    # Pattern 9: Zigzag
    if L5 == ["BIG","SMALL","BIG","SMALL","BIG"]:   add("SMALL", W["zigzagPattern"], "Zigzag BIG→SMALL")
    if L5 == ["SMALL","BIG","SMALL","BIG","SMALL"]: add("BIG",   W["zigzagPattern"], "Zigzag SMALL→BIG")
    # Pattern 10: Long streak reversal
    if streak >= 4:
        bonus = min(95, 75 + (streak - 3) * 5)
        add(breaker, bonus, f"Long Streak ({streak}) Reversal")
    # Pattern 11: Statistical imbalance
    if total >= 10 and imb >= 4:
        tgt = "SMALL" if big_c > sml_c else "BIG"
        add(tgt, W["statisticalImbalance"] + min(20, imb*2), "Statistical Imbalance")
    # Pattern 12: Dot transition (JS dotMap)
    if len(dots6) >= 2:
        if dots6[0] == 3 and dots6[1] == 1: add("SMALL", W["dotTransition"], "Dot 3→1")
        if dots6[0] == 1 and dots6[1] == 3: add("BIG",   W["dotTransition"], "Dot 1→3")
    # Pattern 13: Number sum
    if len(nums8) >= 3:
        s3 = sum(nums8[:3])
        if s3 in (15, 18, 21):
            tgt = "SMALL" if s3 % 2 == 0 else "BIG"
            add(tgt, W["numberSumPattern"], f"Number Sum {s3}")
    # Pattern 14: Oscillation detection
    osc = sum(
        1 for i in range(min(6, len(nums8)-1))
        if (nums8[i] >= 5 and nums8[i+1] <= 4) or (nums8[i] <= 4 and nums8[i+1] >= 5)
    )
    if osc >= 4:
        tgt = "SMALL" if nums8[0] >= 5 else "BIG"
        add(tgt, W["oscillationDetection"], "High Oscillation")
    # Pattern 15: Momentum shift
    prev3 = sizes[3:6]
    recent_big = L3.count("BIG")
    ...
    # ── 4. Dot-size ensemble (JS ensembleVote) ────────────
    dot_corr = {1: {"BIG":3,"SMALL":2}, 2: {"BIG":2,"SMALL":3}, 3: {"BIG":4,"SMALL":1}}
    ...
    # Momentum tweak
    short_big = L3.count("BIG") / 3
    if short_big > 0.66:  scores["SMALL"] += 3
    elif short_big < 0.33: scores["BIG"]   += 3

    # ── 5. Final decision (same as JS finalDecision) ──────
    if streak >= 5:
        final  = breaker
        reason = f"Extreme streak ({streak}) reversal"
    elif imb >= 6 and total >= 15:
        final  = "SMALL" if big_c > sml_c else "BIG"
        ...
        reason = f"Ensemble BIG={int(scores['BIG'])} SMALL={int(scores['SMALL'])}"

    # ── 6. Confidence (same as JS calculateConfidence) ────
    score_diff = abs(scores["BIG"] - scores["SMALL"])
    conf = 50 + min(25, score_diff / 2) + min(15, len(detected))
    if streak >= 4:  conf += 10
    if total  >= 20: conf += 5
    conf = min(99, int(conf))

    # ── 7. Number prediction (JS dotMap + weightedRandom) ─
    dot_num_map = {1: [5,6,7,8], 2: [0,1,2,3,4], 3: [7,8,9]}
    candidates = [
        n for n in dot_num_map.get(last_dot, [])
        if (final == "BIG" and n >= 5) or (final == "SMALL" and n <= 4)
    ]
    if final == "BIG":
        weights = [15, 20, 25, 20, 20]   # 5,6,7,8,9
        pool    = [5, 6, 7, 8, 9]
    else:
        weights = [25, 25, 20, 15, 15]   # 0,1,2,3,4
        pool    = [0, 1, 2, 3, 4]

    def weighted_pick(p, w):
        total_w = sum(w); r = random.random() * total_w; cum = 0
        for v, wt in zip(p, w):
            cum += wt
            if r < cum: return v
        return p[-1]
    ...
    # Second number — different from num1, same side
    alt_pool = [n for n in pool if n != num1]
    num2 = random.choice(alt_pool) if alt_pool else num1
...
def _w_fetch():
    """Fetch WinGo history — multiple API fallbacks."""
    game_code = _wingo_state.get("game_code", "WinGo_30S")

    # API list — একটা না হলে পরেরটা চেষ্টা করবে
    apis = [
        ("POST", "https://dkwin19.com/api/webapi/GetHistoryIssuePage", game_code),
        ("POST", "https://dkwin01.com/api/webapi/GetHistoryIssuePage", game_code),
        ("GET",  f"https://draw.ar-lottery01.com/WinGo/{game_code}/GetHistoryIssuePage.json?ts=", game_code),
        ("GET",  f"https://api.bdgwin.com/WinGo/{game_code}/GetHistoryIssuePage.json?ts=", game_code),
    ]

    for method, url, gc in apis:
        try:
            if method == "POST":
                payload = json.dumps({
                    "pageSize": 100, "pageNo": 1,
                    "gameCode": gc, "language": 0,
                    "random": "", "signature": "", "ts": int(time.time()*1000)
                }).encode("utf-8")
            ...
            result = []
            for x in rows:
                n = int(x.get("number", 0)) % 10
                c = x.get("color", "")
                color = "violet" if "violet" in c else ("green" if "green" in c else "red")
                result.append({
                    "period": str(x.get("issueNumber", x.get("issue", ""))),
                    "number": n,
            ...
            if result:
                logger.info("WinGo data OK from %s: %d rows", url[:40], len(result))
                _wingo_state["history"] = result
                return result

        except Exception as e:
            logger.debug("WinGo API fail %s: %s", url[:40], e)
            continue

    # সব API fail — cached data দাও
    cached = _wingo_state.get("history", [])
    if cached:
        logger.warning("All APIs failed — using cached data (%d rows)", len(cached))
    return cached
...
def main_menu():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("━━━━━━━━━━━━━━━━━━━━━━", callback_data="noop")],
        [InlineKeyboardButton("⚡  WinGo 30S Signal", callback_data="wingo_menu")],
        [InlineKeyboardButton("🎯  WinGo 1Min ULTIMATE", callback_data="wingo1m_menu")],
        [InlineKeyboardButton("━━━━━━━━━━━━━━━━━━━━━━", callback_data="noop")],
...
def wingo1m_menu():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("▶️ সাবস্ক্রাইব", callback_data="w1m_sub"),
         InlineKeyboardButton("⏹ আনসাবস্ক্রাইব", callback_data="w1m_unsub")],
        [InlineKeyboardButton("🔮 এখনই Predict", callback_data="w1m_now")],
        [InlineKeyboardButton("🏠 হোম", callback_data="back_main")],
    ])
    return InlineKeyboardMarkup([
...
async def handle_cb(u, c):
    ...
    if data == "wingo1m_menu":
        await safe_edit(q.message,
            "🎯 *WinGo 1Min ULTIMATE v3.0*\n25+ Pattern AI\n\nসাবস্ক্রাইব করলে প্রতি ১ মিনিটে Signal পাবেন!",
            parse_mode="Markdown", reply_markup=wingo1m_menu()); return

    if data == "w1m_sub":
        _1min_subs.add(uid)
        await safe_edit(q.message,
            "✅ *WinGo 1Min সাবস্ক্রাইব হয়েছে!*\nপ্রতি ১ মিনিটে Signal আসবে 🎯",
            parse_mode="Markdown", reply_markup=wingo1m_menu()); return

    if data == "w1m_unsub":
        _1min_subs.discard(uid)
        await safe_edit(q.message,
            "⏹ *WinGo 1Min আনসাবস্ক্রাইব হয়েছে!*",
            parse_mode="Markdown", reply_markup=wingo1m_menu()); return

    if data == "w1m_now":
        await safe_edit(q.message, "⏳ *Prediction নিচ্ছি...*", parse_mode="Markdown")
        hist = await _fetch_1min()
        if not hist:
            await safe_edit(q.message, "❌ ডেটা পাওয়া যায়নি!", reply_markup=wingo1m_menu()); return
        pred = _ultimate_predict(hist)
        last = hist[0]
        color = "🟢" if pred["size"] == "BIG" else "🔴"
        msg = (
            f"🎯 *WinGo 1Min — ULTIMATE v3.0*\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"📌 Period: `{last['period']}`\n"
            f"🔢 Last: *{last['number']}* ({last['size']})\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🎰 *Predicted Number:* `{pred['number']}`\n"
            f"{color} *Predicted Size:* `{pred['size']}`\n"
            f"📊 *Confidence:* `{pred['confidence']}%`\n"
            f"🧠 *Patterns:* `{pred['patterns']}+`\n"
            f"💡 *Reason:* _{pred['reason']}_\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"⚡ _ULTIMATE 25+ Pattern AI_"
        )
        await safe_edit(q.message, msg, parse_mode="Markdown", reply_markup=wingo1m_menu()); return

    if data == "wingo_menu":
        await safe_edit(q.message,
            "🤖 *ADRIYAN JAFOR AI — WinGo*\n\n"
            "DKWin Real API · 70-layer Analysis\n"
            "RSI · MACD · Pattern Match · Entropy\n\n"
            "👇 Signal নাও",
    ...
    if data == "wingo_predict":
        await safe_edit(q.message,
            "🤖 *ADRIYAN JAFOR AI scanning...*\n`📡 DKWin API · 70 layers · Real-time data`",
            parse_mode="Markdown")
        loop = asyncio.get_running_loop()
        ...
        alts     = pred["numAlt"]
        conf     = pred["conf"]
        sig_color = "BIG  ৫~৯" if pred["sig"]=="BIG" else "SMALL  ০~৪"
        bar_fill  = round(conf / 10)
        ...
        pG = pred.get("pG", 45)
        pV = pred.get("pV", 10)
        conf_icon = "🔥" if conf>=87 else ("💪" if conf>=80 else ("✅" if conf>=70 else ("⚠️" if conf>=60 else "❓")))
        ...
        txt = (
            ...
            f"━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"{se}  *{pred['sig']}*  {te}  *{pred['tc'].upper()}*\n\n"
            f"🎯  *NO: {pred['numTop']}*  |  Alt: `{alts[0]}`  `{alts[1]}`  `{alts[2]}`\n\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"{conf_icon}  `{conf_bar}`  *{conf}%*\n"
            f"🔴 {pR}%  🟢 {pG}%  🟣 {pV}%\n\n"
