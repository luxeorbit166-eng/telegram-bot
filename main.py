...
def _w_fetch():
    """Fetch WinGo history from DKWin real API."""
    game_code = _wingo_state.get("game_code", "WinGo_30S")
    try:
        payload = json.dumps({
            "pageSize": 100, "pageNo": 1,
            "gameCode": game_code, "language": 0,
        ...
        logger.debug("DKWin API error: %s", e)
    return _wingo_state.get("history", [])
