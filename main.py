...
def _ultimate_predict(history: list) -> dict:
    if len(history) < 3:
        return {"size": "BIG", "number": 5, "confidence": 65, "patterns": 0, "reason": "Insufficient data"}
    ...
    def add(target, score): scores[target] += score; nonlocal detected; detected += 1

    # 25+ Patterns
    L2, L3, L4, L5 = sizes[:2], sizes[:3], sizes[:4], sizes[:5]

    if L2 == ["SMALL","SMALL"]: add("BIG", 87)
    if L3 == ["BIG","BIG","BIG"]: add("SMALL", 70)
    if L4.count("BIG") == 4: add("SMALL", 85)
    if L4.count("SMALL") == 4: add("BIG", 85)
    if L5.count("BIG") == 5: add("SMALL", 92)
    if L5.count("SMALL") == 5: add("BIG", 92)
    if L4 == ["BIG","SMALL","BIG","SMALL"]: add("BIG", 75)
    if L4 == ["SMALL","BIG","SMALL","BIG"]: add("SMALL", 75)
    if numbers[0] == 0: add("BIG", 65)
    if numbers[0] == 9: add("SMALL", 60)
    if numbers[0] in [0,9]: add("BIG" if numbers[0]==0 else "SMALL", 73)
    ...
    if streak >= 4: add(breaker, min(95, 75 + (streak-3)*5))

    # Statistical imbalance
    big_c = sizes[:20].count("BIG")
    sml_c = sizes[:20].count("SMALL")
    imb = abs(big_c - sml_c)
    if imb >= 4: add("SMALL" if big_c > sml_c else "BIG", 40 + imb*2)

    # Number clusters
    from collections import Counter
    nc = Counter(numbers[:5])
    for num, cnt in nc.items():
        if cnt >= 3: add("SMALL" if num >= 5 else "BIG", 77)

    # Zigzag
    if L5 == ["BIG","SMALL","BIG","SMALL","BIG"]: add("SMALL", 69)
    if L5 == ["SMALL","BIG","SMALL","BIG","SMALL"]: add("BIG", 69)

    # Momentum
    short = sizes[:3].count("BIG") / 3
    if short > 0.66: scores["SMALL"] += 5
    elif short < 0.33: scores["BIG"] += 5
    ...
    if streak >= 5:
        ...
        reason = f"Ensemble BIG={scores['BIG']} SMALL={scores['SMALL']}"

    # Confidence
    conf = 50 + min(25, abs(scores["BIG"]-scores["SMALL"])/2) + min(15, detected)
    if streak >= 4: conf += 10
    conf = min(99, int(conf))

    # Predict number
    if final == "BIG":
        num = random.choice([5,6,6,7,7,8,9])
    else:
        num = random.choice([0,1,2,2,3,3,4])

    return {"size": final, "number": num, "confidence": conf,
            "patterns": detected, "reason": reason, "streak": streak}

async def _1min_broadcast(bot):
    global _1min_history
...
def main():
    ...
    if app.job_queue:
        ...
        app.job_queue.run_repeating(
            lambda ctx: asyncio.ensure_future(_1min_broadcast(ctx.bot)),
