...
def yt_dl(url, job):
    ...
    if any(d in u for d in ("youtube.com","youtu.be","shorts.google.com")):
        strategies = [
            # Try 1080p first
            ("bv*[height<=1080][ext=mp4]+ba[ext=m4a]/bv*[height<=1080]+ba/b[height<=1080][ext=mp4]",
             ["--user-agent",MUA,"--extractor-args","youtube:player_client=android,ios",
        ...
        strategies = [
            ...
            ("best[height<=720]/best", ["--user-agent",DUA,"--no-check-certificates"]),
        ]
...
def process_enhance(inp):
    ...
    # Smart 4K upscale — portrait vs landscape
    if w < h:
        # Portrait (TikTok/Reels style) → 2160x3840
        target_h = 3840
        target_w = 2160
    else:
    ...
    # Only upscale if source is smaller
    if h < target_h:
        scale_f = (
            f"scale={target_w}:{target_h}:"
            f"force_original_aspect_ratio=decrease:"
            f"flags=lanczos+accurate_rnd+full_chroma_inp,"
    ...
    vf = (
        # Stage 1: Deep noise removal
        f"hqdn3d=3:3:6:6,"
        # Stage 2: 4K upscale
        f"{scale_f}"
        # Stage 3: Cinematic color grading
        f"eq=contrast=1.10:brightness=0.018:saturation=1.30:gamma=0.93,"
        # Stage 4: Warm tone + hue
        f"hue=h=1.2:s=1.08,"
        # Stage 5: Pro sharpening (luma + chroma)
        f"unsharp=7:7:1.5:5:5:0.6,"
        # Stage 6: Cinematic vignette
        f"vignette=PI/4.2,"
        # Stage 7: Film grain (subtle texture)
    ...
    if has_audio(inp):
        af = (
            ...
            "equalizer=f=80:width_type=o:width=1:g=1.0"
        )
        cmd = ["ffmpeg","-y","-i",inp,
               "-vf", vf, "-af", af,
...
def process_tt_mode(inp):
    """
    TikTok Mode ULTRA — 8-layer copyright bypass:
    Layer 1: 9:16 portrait crop/pad
    Layer 2: TikTok vivid warm color profile
    Layer 3: Microscopic video rotation (unique fingerprint)
    Layer 4: Subtle zoom (1.5% scale change)
    Layer 5: Mobile-optimized sharpening
    Layer 6: Audio pitch + tempo shift
    Layer 7: Multi-band EQ (frequency fingerprint change)
    Layer 8: Full metadata + stream ID strip
    """
    out = tmp_path(".mp4")

    # Random values for unique fingerprint every time
    rot   = round(random.uniform(0.2, 0.5), 2)
    zoom  = round(random.uniform(1.005, 1.015), 4)
    br    = round(random.uniform(0.008, 0.018), 3)
    sat   = round(random.uniform(1.18, 1.25), 3)
    pitch = round(random.uniform(1.5, 2.5), 1)
    hue_shift = round(random.uniform(1.5, 3.0), 1)
    ...
    vf = (
        ...
        f"rotate={rot}*PI/180:fillcolor=black@0:ow=iw:oh=ih,"
        f"crop=1076:1916:2:2,"
        f"eq=contrast=1.06:brightness={br}:saturation={sat}:gamma=0.97,"
        f"hue=h={hue_shift}:s=1.04,"
        f"unsharp=3:3:0.9:3:3:0.3"
    )
    ...
    if has_audio(inp):
        af = (
            f"asetrate=44100*{1+(pitch/1000)},"
            f"atempo={1-(pitch/2000)},"
            f"equalizer=f=12000:width_type=o:width=2:g=1.5,"
            f"equalizer=f=8000:width_type=o:width=1.5:g=0.8,"
            f"equalizer=f=100:width_type=o:width=1:g=-0.8,"
...
MODE_LABELS = {
    "enhance": "✨ Enhance 4K ULTRA",
    "yt_mode": "📺 YouTube Mode",
    "tt_mode": "📱 TikTok Mode",
    "ig_mode": "📸 Instagram Mode",
    "all_mode": "🚀 সব একসাথে (4K+YT+TT+IG)",
}
...
MODE_CAPTIONS = {
    "enhance": (
        ...
        "• Warm tone + hue correction\n"
        "• Pro sharpening (luma+chroma)\n"
        "• Cinematic vignette\n"
        "• Film grain texture\n"
    ...
    "tt_mode": (
        "📱 *TikTok Mode ULTRA — Copyright Safe*\n\n"
        "🛡️ 8-layer bypass:\n"
        "• 9:16 portrait (1080×1920)\n"
        "• Micro-rotation + zoom fingerprint\n"
        "• Vivid warm color + hue shift\n"
        "• Audio pitch + EQ bypass\n"
        "• Full metadata strip\n"
        "✅ FYP-ready!"
    ),
    "ig_mode": (
    ...
    "all_mode": (
        "🚀 *সব Processing সম্পন্ন!*\n\n"
        "✅ 4K Enhance\n"
        "✅ YouTube Mode (Copyright Safe)\n"
        "✅ TikTok Mode (FYP-ready)\n"
        "✅ Instagram Mode"
    ),
}
...
def edit_btns(job):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🚀 সব একসাথে (4K+YT+TT+IG)", callback_data=f"do_all_mode_{job}")],
        [InlineKeyboardButton("✨ Enhance 4K", callback_data=f"do_enhance_{job}"),
...
✨ *Enhance ULTRA:*
▸ সিনেমা মানের processing
▸ Smart 1080p Upscale
▸ Cinematic color grading

🛡️ *Platform Mode (Copyright Safe):*
▸ 📺 YouTube Mode (16:9)
▸ 📱 TikTok ULTRA (9:16) — 8-layer bypass
▸ 📸 Instagram Mode (4:5)

🎯 *WinGo AI Prediction:*
▸ 35-layer analysis engine
▸ Color + Number prediction

━━━━━━━━━━━━━━━━━━━━━━
💡 *ব্যবহার:*
🔗 Link পাঠান → Download
🎥 ভিডিও পাঠান → Edit options
👇 নিচের বাটন চাপুন → Menu
""".strip()
...
async def handle_msg(u, c):
    ...
    if urls:
        ...
        # ── AUTO PROCESS: Platform অনুযায়ী ──
        is_yt   = any(d in u2 for d in ("youtube.com","youtu.be"))
        is_tt   = any(d in u2 for d in ("tiktok.com","vm.tiktok","vt.tiktok"))
        is_ig   = any(d in u2 for d in ("instagram.com","instagr.am"))
        is_fb   = any(d in u2 for d in ("facebook.com","fb.watch","fb.com"))

        if is_yt:
            proc2       = process_yt_mode
            proc2_label = "YouTube Copyright Bypass"
            final_note  = "✅ Enhance 4K ULTRA\n✅ YouTube Copyright Bypass\n✅ Metadata Strip\n🎯 YouTube-ready!"
        elif is_tt:
            proc2       = None  # special 3-stage below
            proc2_label = "TikTok + YouTube Bypass"
            final_note  = "✅ Enhance 4K ULTRA\n✅ TikTok 8-layer Bypass\n✅ YouTube Copyright Bypass\n✅ Not Eligible Fix\n🎯 FYP-ready!"
        elif is_ig:
            proc2       = process_ig_mode
            proc2_label = "Instagram Optimize"
            final_note  = "✅ Enhance 4K ULTRA\n✅ Instagram 4:5 Optimize\n✅ Metadata Strip\n🎯 Instagram-ready!"
        elif is_fb:
            proc2       = process_yt_mode
            proc2_label = "Facebook Copyright Bypass"
            final_note  = "✅ Enhance 4K ULTRA\n✅ Facebook Copyright Bypass\n✅ Metadata Strip\n🎯 Facebook-ready!"
        else:
            proc2       = process_yt_mode
            proc2_label = "Copyright Bypass"
            final_note  = "✅ Enhance 4K ULTRA\n✅ Copyright Bypass\n✅ Metadata Strip"

        await safe_edit(st,
            f"✅ *ডাউনলোড হয়েছে!* `{size_mb:.1f} MB`\n\n"
            f"🔄 *Auto Processing...*\n"
            f"⏳ Stage 1/2 — Enhance 4K...",
            parse_mode="Markdown")

        # Stage 1: Enhance 4K
        enhanced, _ = await loop.run_in_executor(executor, process_enhance, filepath)
        src = enhanced if enhanced else filepath

        await safe_edit(st,
            f"🔄 *Auto Processing...*\n"
            f"✅ Stage 1/2 — Enhance 4K done!\n"
            f"⏳ Stage 2/2 — {proc2_label}...",
            parse_mode="Markdown")

        # Stage 2: Platform-specific bypass
        if is_tt:
            # TikTok: TikTok bypass → YouTube bypass (3 stage total)
            await safe_edit(st,
                f"🔄 *Auto Processing...*\n"
                f"✅ Stage 1/3 — Enhance 4K done!\n"
                f"⏳ Stage 2/3 — TikTok Bypass...",
                parse_mode="Markdown")
            tt_out, _ = await loop.run_in_executor(executor, process_tt_mode, src)
            src2 = tt_out if tt_out else src

            await safe_edit(st,
                f"🔄 *Auto Processing...*\n"
                f"✅ Stage 1/3 — Enhance 4K done!\n"
                f"✅ Stage 2/3 — TikTok Bypass done!\n"
                f"⏳ Stage 3/3 — YouTube Copyright Bypass...",
                parse_mode="Markdown")
            out2, _ = await loop.run_in_executor(executor, process_yt_mode, src2)
            final = out2 if out2 else src2

            # Cleanup
            for p in [enhanced, tt_out]:
                if p and p != final:
                    try: Path(p).unlink()
                    except Exception: pass
        else:
            out2, _ = await loop.run_in_executor(executor, proc2, src)
            final = out2 if out2 else src
            if enhanced and enhanced != final:
                try: Path(enhanced).unlink()
                except Exception: pass

        final_mb = Path(final).stat().st_size / 1024 / 1024 if Path(final).exists() else size_mb

        await safe_edit(st,
            f"⏫ *Upload হচ্ছে...* `{final_mb:.1f} MB`",
            parse_mode="Markdown")
        ...
        try:
            with open(final,"rb") as f:
                await msg.reply_video(f, read_timeout=300, write_timeout=300, connect_timeout=60,
                    caption=(
                        f"✅ *{md_escape(title or 'সম্পন্ন')}*\n\n"
                        f"🎬 {plat} | 📦 `{final_mb:.1f} MB`\n\n"
                        f"🔥 *Auto Processing:*\n"
                        f"{final_note}"
                    ),
                    parse_mode="Markdown",
...
async def handle_cb(u, c):
    ...
    if data == "wingo_predict":
        ...
        # Recent history mini display
        hist_mini = ""
        recent = _wingo_state.get("history",[])[:8]
        if recent:
            hist_mini = " ".join(
                ("🔴" if h["size"]=="BIG" else "🟢") for h in recent
            )
        ...
        txt = (
            ...
            f"`{bar}` {conf_icon} *{pred['conf']}%* — {pred['conf_label']}\n\n"
            f"🎨 *Color:* {te} *{pred['tc'].upper()}*\n"
            f"   🔴 {pred['pR']}%  🟢 {pred['pG']}%  🟣 {pred['pV']}%\n\n"
            f"🎲 *Number:* `{pred['numTop']}`\n"
            f"   Alt: `{pred['numAlt'][0]}` `{pred['numAlt'][1]}` `{pred['numAlt'][2]}`\n\n"
            f"📊 Layer: BIG `{pred['big_layers']}` / SMALL `{pred['small_layers']}` / 35\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🕹 Recent: {hist_mini}\n\n"
            f"⚠️ _Random game — guarantee নেই_\n\n"
            "👇 *Result কি হয়েছিল?*"
        )
    ...
    # ── Video Processing ──
    mode = job_id = None
    for m in ["all_mode","enhance","yt_mode","tt_mode","ig_mode"]:
        pfx = f"do_{m}_"
    ...
    # ── ALL MODE: process all 4 versions ──
    if mode == "all_mode":
        await safe_edit(q.message,
            "🚀 *সব Processing শুরু হচ্ছে...*\n\n"
            "⏳ ১/৪ — Enhance 4K...",
            parse_mode="Markdown")
        loop = asyncio.get_running_loop()
        all_modes = [
        ...
        success = 0
        for i, (m, label) in enumerate(all_modes, 1):
            try:
                await safe_edit(q.message,
                    f"🚀 *Processing...*\n\n"
                    f"⏳ {i}/4 — {label}...\n"
                    f"{'█'*i}{'░'*(4-i)} {i*25}%",
                    parse_mode="Markdown")
                out_path, err = await loop.run_in_executor(executor, PROCESSORS[m], inp_path)
                if out_path:
                    ...
                    with open(out_path,"rb") as f:
                        await q.message.reply_video(f,
                            ...
                            caption=MODE_CAPTIONS.get(m,"✅ সম্পন্ন!"),
                            parse_mode="Markdown")
                    try: Path(out_path).unlink()
                    except Exception: pass
                    success += 1
            except Exception as e:
                logger.warning("all_mode %s error: %s", m, e)
        await safe_edit(q.message,
            f"✅ *সম্পন্ন! {success}/4 ভিডিও তৈরি হয়েছে!*\n\n"
            f"{'✅ Enhance 4K' if success>=1 else '❌ Enhance'}\n"
            f"{'✅ YouTube Mode' if success>=2 else '❌ YouTube'}\n"
            f"{'✅ TikTok Mode' if success>=3 else '❌ TikTok'}\n"
            f"{'✅ Instagram Mode' if success>=4 else '❌ Instagram'}",
            parse_mode="Markdown", reply_markup=main_menu())
        return
...
def main():
    ...
    print("╔════════════════════════════════════════════╗")
    print("║  🎬  Video Bot v42 Premium চালু!           ║")
    print("║  ✨  Enhance Ultra PREMIUM                  ║")
    print("║  📺  YouTube + 🛡️ Copyright Safe            ║")
    print("║  📱  TikTok ULTRA + 8-layer bypass          ║")
