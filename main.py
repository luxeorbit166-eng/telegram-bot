...
async def cmd_test(u, c):
    """API test — সব endpoint check করে result দেখায়"""
    msg = await safe_reply(u.message, "⏳ *API test চলছে...*", parse_mode="Markdown")
    ...
    lines.append("📡 *WinGo 1Min APIs:*")
    for url in min_apis:
        try:
            import urllib.request as _r
            r = _r.urlopen(
                _r.Request(url, headers={"User-Agent": "Mozilla/5.0"}),
                timeout=8
            )
            data = json.loads(r.read())
            rows = data.get("data", {}).get("list", [])
            cnt  = len(rows)
            short = url.split("/")[2]
            lines.append(f"  ✅ `{short}` → *{cnt} rows*")
        except Exception as e:
            short = url.split("/")[2]
            lines.append(f"  ❌ `{short}` → `{str(e)[:40]}`")

    # ── 30s API test ──────────────────────────────────────
    lines.append("\n📡 *WinGo 30s APIs:*")
    s30_apis = [
        f"https://draw.ar-lottery01.com/WinGo/WinGo_30S/GetHistoryIssuePage.json?ts={ts}",
        f"https://api.bdgwin.com/WinGo/WinGo_30S/GetHistoryIssuePage.json?ts={ts}",
    ]
    for url in s30_apis:
        try:
            r = _r.urlopen(
    ...
    hist1 = await _fetch_1min()
    hist3 = await loop.run_in_executor(None, _w_fetch)

    lines.append("\n━━━━━━━━━━━━━━━━━━━━━━━")
    if hist1:
        last = hist1[0]
        lines.append(f"🎯 *1Min Latest:* `{last['period'][-6:]}` → *{last['number']}* ({last['size']})")
    else:
        lines.append("❌ *1Min: data পাওয়া যায়নি*")

    if hist3:
        last = hist3[0]
        lines.append(f"🎯 *30s Latest:* `{last['period'][-6:]}` → *{last['number']}* ({last['size']})")
    else:
        lines.append("❌ *30s: data পাওয়া যায়নি*")

    result_txt = "\n".join(lines)
    try:
        await msg.edit_text(result_txt, parse_mode="Markdown")
    except Exception:
        await safe_reply(u.message, result_txt, parse_mode="Markdown")
...
async def handle_cb(u, c):
    ...
    if data == "wingo_predict":
        await safe_edit(q.message,
            "⏳ *ULTIMATE AI scanning...*\n`📡 25+ Patterns · Real-time data`",
            parse_mode="Markdown")
        loop = asyncio.get_running_loop()
        history = await loop.run_in_executor(executor, _w_fetch)
        if not history:
            await safe_edit(q.message,
                "❌ *ডেটা পাওয়া যায়নি!*\n\n`/test` দিয়ে API check করো।",
                parse_mode="Markdown", reply_markup=wingo_menu()); return

        _wingo_state["history"] = history
        pred = _ultimate_predict(history)
